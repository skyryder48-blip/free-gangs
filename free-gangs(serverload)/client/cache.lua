...
-- ============================================================================
-- GRAFFITI SPRAY TRACKING
-- ============================================================================

local sprayCounts = {}
local sprayResetTime = 0

---Track a spray
function FreeGangs.Client.Cache.TrackSpray()
    local now = GetGameTimer()
    local cycleMs = FreeGangs.Config.Activities.Graffiti.CycleDurationHours * 3600000
    
    if now > sprayResetTime then
        sprayCounts = {}
        sprayResetTime = now + cycleMs
    end
    
    sprayCounts[#sprayCounts + 1] = now
end

---Get spray count this cycle
---@return number
function FreeGangs.Client.Cache.GetSprayCount()
    local now = GetGameTimer()
...
---Check if can spray
---@return boolean
function FreeGangs.Client.Cache.CanSpray()
    return FreeGangs.Client.Cache.GetSprayCount() < FreeGangs.Config.Activities.Graffiti.MaxSpraysPerCycle
end
